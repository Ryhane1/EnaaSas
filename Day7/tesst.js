...
function rechercherTache() {
    let id = Number(prompt("Entrez l'ID de la tâche : "));
    
    for (let i = 0; i < tasks.length; i++) {
        if (tasks[i].id === id) {
     
            console.log(`${tasks[i].id}. ${tasks[i].description}. ${tasks[i].isDone}`);
...
function modifierTache() {
    let id = Number(prompt("Entrez l'ID de la tâche à modifier : "));
    let trouve = false;
    for (let i = 0; i < tasks.length; i++) {
        if (tasks[i].id === id) {
